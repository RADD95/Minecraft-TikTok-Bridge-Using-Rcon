...
const queue = require('./queue');  // Cola de comandos
...
class ActionsService {
  ...
  async handleEvent(type, data) {
    ...
    for (const action of actions) {
      ...
      // Si hay un gift específico, NO ejecutar los genéricos (trigger vacío)
      ...
      // Expandir combos por repeatcount si la acción lo pide
      ...
      // DECISIÓN: cola o directo
      const shouldQueue = (action.useQueue ?? false) || !rconService.isConnected();
      const sourceName =
        action.name ||
      ...
      if (shouldQueue) {
        ...
        // Ejecución directa (solo si hay RCON)
